...
界面会自己识别这种情况：服务把「启动时加载的后端代码指纹」和磁盘上的现状做比对，发现不一致就在页面顶部挂一条**「后端代码已更新，当前服务仍在运行旧版本」**的横幅，点里面的「立即重启」即可 —— 后端会先拉起一个脱离父进程的新服务（沿用同一端口），再退出旧进程，页面自动刷新，不需要你去 Finder / Dock 里操作。也可以按老办法：右上角「关闭服务」，再重新双击 `app/MacKit.command`。
